...
const [defaultAdmin, alice, upgrader, charlie] = getSigners();
describe.only('Upgradeable', () => {
  ...
  beforeEach(async () => {
    ...
    await time.setTo(10, api);
    const v0 = await new TFlipperV0Deployer(api, defaultAdmin).new(false);
  ...
  describe('v0', () => {
    ...
    describe('when upgrading code', () => {
      ...
      beforeEach(async () => {
        ...
        const resOriginal = (await api.query.contracts.contractInfoOf(ctx.flipper.address)).toHuman() as { codeHash: string };
        originalHash = resOriginal.codeHash;
        const res = (await api.query.contracts.contractInfoOf(flipperv1.address)).toHuman() as { codeHash: string };
      ...
      describe('after upgrade to v1', () => {
        ...
        it(`code hash was updated`, async () => {
          const res = (await api.query.contracts.contractInfoOf(ctx.flipper.address)).toHuman() as { codeHash: string };
